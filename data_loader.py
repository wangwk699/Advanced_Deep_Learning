...
负责加载本地 Food_Multimodal 数据集中的 5 个已知类，并从 HuggingFace Food-101
中选取若干与已知类不重合的类别作为开放集 unknown classes。

本版本仅使用英文类别名构造 CLIP prompt，不再使用中文提示词。
"""
...
def get_class_prompts(class_names: List[str], ensemble: bool = True, index: int = 0) -> List[str]:
    """为 CLIP zero-shot 分类生成英文文本提示。
    ...
    for class_name in class_names:
        food_name = class_name.replace("_", " ")
        if ensemble:
            prompts.extend(template.format(food_name) for template in templates)
        else:
            prompts.append(templates[index].format(food_name))

    return prompts


def get_unknown_class_prompts(ensemble: bool = True) -> List[str]:
    """获取 Food-101 unknown 候选类的英文 prompts。"""
    return get_class_prompts(UNKNOWN_CLASSES_FOOD101, ensemble=ensemble)
